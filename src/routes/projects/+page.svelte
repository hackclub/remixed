...
<div class="relative z-2 flex h-screen w-screen items-center justify-center">
	<div class="grid grid-cols-3 gap-16">
		<div
			class="flex w-60 flex-col items-center gap-8 rounded-3xl border-4 border-[#8B81FF] bg-text p-8 shadow-simple"
		>
			<img src="/dashboard/plus_fill.png" alt="plus" class="m-4 w-16" />
			<h2 class="text-center font-jua text-3xl text-[#E2BEFF] text-shadow-lg/30">
				Create new project
			</h2>
		</div>
		{#each userProjects as proj}
			<div
				class="flex w-60 flex-col rounded-3xl border-4 border-[#8B81FF] bg-text p-4 shadow-simple"
			>
				<img
					src={proj.coverArt}
					alt="cover"
					class="h-30 w-full rounded-xl border-4 border-[#E2BEFF] object-cover px-4"
				/>
				<div class="flex grow flex-col justify-end">
					<h2 class=" line-clamp-1 font-jua text-4xl text-[#E2BEFF] text-shadow-lg/30">
						{proj.title}
					</h2>
					<p class="line-clamp-2 font-jua text-[#E2BEFF]">{proj.description}</p>
				</div>
			</div>
		{/each}
	</div>
